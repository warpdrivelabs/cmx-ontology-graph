...
export function chooseSides(a, b) {
    ...
    if (Math.abs(dx) >= Math.abs(dy))
        return dx >= 0 ? { sa: 'R', sb: 'L' } : { sa: 'L', sb: 'R' };
    return dy >= 0 ? { sa: 'B', sb: 'T' } : { sa: 'T', sb: 'B' };
}
function anchorPt(r, s) {
    if (s === 'R')
        return { x: r0(r.x + r.w), y: r0(r.y + r.h / 2) };
    if (s === 'L')
        return { x: r0(r.x), y: r0(r.y + r.h / 2) };
    if (s === 'T')
        return { x: r0(r.x + r.w / 2), y: r0(r.y) };
...
function astar(start, goal, xs, ys, obst) {
    ...
    if (!ixOf.has(start.x) || !iyOf.has(start.y) || !ixOf.has(goal.x) || !iyOf.has(goal.y))
        return null;
    const K = (x, y) => x + ':' + y;
    ...
    while (open.size > 0 && guard++ < 40000) {
        ...
        const xr = xs[ix + 1];
        if (xr !== undefined)
            cand.push({ x: xr, y: cur.y, d: 'H' });
        const xl = xs[ix - 1];
        if (xl !== undefined)
            cand.push({ x: xl, y: cur.y, d: 'H' });
        const yd = ys[iy + 1];
        if (yd !== undefined)
            cand.push({ x: cur.x, y: yd, d: 'V' });
        const yu = ys[iy - 1];
        if (yu !== undefined)
            cand.push({ x: cur.x, y: yu, d: 'V' });
        for (const nb of cand) {
            const nKey = K(nb.x, nb.y);
            if (closed.has(nKey))
                continue;
            if (blocked({ x: cur.x, y: cur.y }, { x: nb.x, y: nb.y }, obst))
                continue;
...
export function routeAnchored(pa, sa, pb, sb, endA, endB, others) {
    ...
    let mids = null;
    if (!ptInside(sPt, obst) && !ptInside(gPt, obst))
        mids = astar(sPt, gPt, xs, ys, obst);
    const raw = mids ? [pa, sPt, ...mids, gPt, pb] : fallback(pa, sPt, gPt, pb);
    return simplify(raw);
}
/** 布线主入口（节点级：几何朝向侧中点）。others = 除两端外的其余节点盒。 */
export function routeEdge(a, b, others) {
    if (Math.abs(a.x - b.x) < 0.5 && Math.abs(a.y - b.y) < 0.5 && a.w === b.w && a.h === b.h)
        return selfLoop(a);
    const { sa, sb } = chooseSides(a, b);
    return routeAnchored(anchorPt(a, sa), sa, anchorPt(b, sb), sb, a, b, others);
}
/** 折点序列 → 圆角 SVG path。 */
export function toPath(pts, radius = 7) {
    if (pts.length < 2)
        return '';
    const p0 = pts[0];
    if (pts.length === 2) {
...
export function polyMidpointOriented(pts) {
    if (pts.length === 0)
        return { x: 0, y: 0, vertical: false };
    if (pts.length === 1)
        return { x: pts[0].x, y: pts[0].y, vertical: false };
    const seg = [];
    let total = 0;
