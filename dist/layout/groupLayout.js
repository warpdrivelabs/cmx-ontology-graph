 ...
 */
import { nodeHeight } from './layout.js';
const PAD = 16;
const HEADER = 34;
const GAP = 26;
const BOX_W = 220;
...
function layoutGroup(g, ox, oy, cfg, collapsed) {
    ...
    for (const leaf of g.leaves)
        children.push({ w: cfg.nodeW, h: nodeHeight(leaf, cfg), kind: 'leaf', node: leaf });
    const boxes = [];
    const pos = {};
    const childX = ox + PAD;
    ...
    for (const c of children) {
        if (c.kind === 'leaf') {
            ...
            const dx = x;
            const dy = y;
            for (const b of c.sub.boxes)
                boxes.push({ ...b, rect: { x: b.rect.x + dx, y: b.rect.y + dy, w: b.rect.w, h: b.rect.h } });
...
/** 顶层布局：所有顶级域组货架式铺开。 */
export function groupLayout(def, cfg, collapsed) {
    ...
    const resolveKey = (nodeId) => {
        ...
        for (let i = 0; i < path.length; i++) {
            const key = path.slice(0, i + 1).join('/');
            if (isCollapsed(collapsed, key))
                return key;
        }
        return nodeId;
    };
    const rectOf = (key) => pos[key] || (boxByKey.get(key) ? boxByKey.get(key).rect : undefined);
    return { boxes, pos, resolveKey, rectOf, width: maxRight + PAD, height: maxBottom + PAD };
}
/** 是否有意义地分组（≥2 个含叶的最深组）。否则宿主回退扁平图。 */
export function groupingActive(def) {
    const keys = new Set();
    for (const n of def.nodes || []) {
        const p = pathOf(n);
        keys.add(p.join('/'));
...
/** 所有分组键（工具栏「全部展开/收起」用）。 */
export function allGroupKeys(def) {
    const keys = new Set();
    for (const n of def.nodes || []) {
        const p = pathOf(n);
        for (let i = 0; i < p.length; i++)
            keys.add(p.slice(0, i + 1).join('/'));
    }
    return [...keys];
}
