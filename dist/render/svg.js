...
import { routeEdge, routeAnchored, toPath, polyMidpoint, polyMidpointOriented } from '../layout/route.js';
/** 该锚点是否为当前高亮锚点。 */
...
function rowPorts(ctx, propApi, rowIdx) {
    ...
    return (`<circle class="og-port${hl('L')}" data-port="${ctx.pid}" data-prop="${pa}" data-side="L" cx="${ctx.rx}" cy="${cy}" r="4.5"/>` +
...
function interfaceCard(n, r, sel, hot) {
    ...
    const ports = `<circle class="og-port${hl('T')}" data-port="${nid}" data-side="T" cx="${cx}" cy="${r.y}" r="4.5"/>` +
...
function renderRowIndexOf(props, propApi, maxRows, depth, startIdx) {
    ...
    for (const p of shown) {
        if (p.apiName === propApi)
            return idx;
        idx += 1;
    }
    if (depth === 0 && scalars.length > shown.length)
        idx += 1; // +N more 行
    for (const lv of levels) {
        if (lv.apiName === propApi)
            return idx;
        idx += 1;
...
function propAnchor(rect, node, cfg, propApi, side) {
    if (!propApi || !node || !node.properties)
        return null;
...
export function renderGrouped(def, gl, cfg, st) {
    const nodeById = new Map(def.nodes.map((n) => [n.id, n]));
    ...
    for (const e of def.edges || []) {
        ...
        if (aK === e.source && bK === e.target) {
            ...
            if (cur)
                ...
                bundles.set(k, { a: aK, b: bK, count: 1 });
        }
    }
    let bundleEdges = '';
